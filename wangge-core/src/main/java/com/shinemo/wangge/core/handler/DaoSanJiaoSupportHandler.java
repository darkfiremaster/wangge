...
import com.shinemo.stallup.domain.params.DaosanjiaoSupportParams;
...
@Slf4j
@Component
public class DaoSanJiaoSupportHandler implements UrlRedirectHandler{
	...
	public String getUrl(UrlRedirectHandlerRequest request) {
		...
		DaosanjiaoSupportParams bizParams = request.getBizParams();

		Map<String, Object> formData = new HashMap<>();
		formData.put("mobile", mobile);
		formData.put("menu", bizParams.getMenu());
		formData.put("timestamp", timestamp);

		String paramStr = EncryptUtil.buildParameterString(formData);
		log.info("[getUrl] 倒三角支撑,formData:{}",formData);
