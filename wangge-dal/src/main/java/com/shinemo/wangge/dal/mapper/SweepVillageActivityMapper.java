package com.shinemo.wangge.dal.mapper;

import com.shinemo.common.db.dao.BaseMapper;
import com.shinemo.sweepvillage.domain.model.SweepVillageActivityDO;
import com.shinemo.sweepvillage.domain.query.SweepVillageActivityQuery;
/**
 * @Author shangkaihui
 * @Date 2020/6/3 10:46
 * @Desc
 */
public interface SweepVillageActivityMapper extends BaseMapper<SweepVillageActivityQuery, SweepVillageActivityDO> {

    SweepVillageActivityDO getUserLastVillage(SweepVillageActivityQuery sweepVillageActivityQuery);
}
